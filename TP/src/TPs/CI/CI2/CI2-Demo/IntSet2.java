...
public class IntSet2 {
   ...
   */
  public IntSet2(IntSet2 original) {
  ...
  private class IntSetGenerator implements Iterator<Integer> {

    //Note étudiants:
    //Pour savoir quel élément a déjà été généré, et quels éléments restent à générer, on a besoin de connaître
    //l'indice du dernier élément généré par le générateur
    private int lastRetrievedIndex;

    public IntSetGenerator() {
      lastRetrievedIndex = 0;
    }

    @Override
    public boolean hasNext() {
      return lastRetrievedIndex < els.size();
    }

    @Override
    public Integer next() throws NoSuchElementException {
      if(!hasNext())
        throw new NoSuchElementException("IntSet2.IntSetGenerator.next()");

      Integer currentElement = els.get(lastRetrievedIndex);
      lastRetrievedIndex++;
